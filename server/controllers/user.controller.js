...
export async function refreshController(req, res) {
    try {
        ...
        // const refreshToken = req.localStorage.getItem("refreshToken")
        ...
        try {
            ...
            console.log("Decoded refreshToken:", verifyToken);
        } catch (err) {
        ...
        if (!currUser || currUser.refresh_token !== refreshToken) {
            return res.status(401).json({
                message: "Invalid Refresh Token (DB mismatch)",
                error: true,
                success: false,
            });
        }
